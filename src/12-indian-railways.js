...
export function railwayReservation(passengers, trains) {
  if (!passengers || !trains || !Array.isArray(passengers) || !Array.isArray(trains) || passengers.length === 0 || trains.length === 0) {
    return []
  }
  ...
  for (let i = 0; i < passengers.length; i++) {
    ...
    for (let j = 0; j < trains.length; j++) {
      ...
      if (train.trainNumber === passenger.trainNumber) {
        ...
        else if (fallback && train.seats[fallback] !== undefined && train.seats[fallback] > 0) {
